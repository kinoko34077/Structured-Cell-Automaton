...
def _show_cached_figure(key, signature, builder):
    ...
    figure = get_cached_figure(st.session_state, key, signature, builder)
...
if st.button("進化 → 評価 → 発話チェック"):
    ...
    for gen in range(5):
        ...
        if oz.should_emit():
            ...
            emitted_in_action = bool(emitted)
            break
        generation = evolve_generation_with_tags(generation, cell_dict)  # ← 意味タグベース交叉

    if emitted_in_action:
        st.success("進化・評価・発話チェックが完了しました。発話構文を生成しました。")
    else:
        st.info("進化・評価・発話チェックは完了しましたが、発話構文はありませんでした。")
...
_show_cached_figure(
    "sca_cluster_figure",
    ("cluster", _syntax_signature(syntax_pool), tuple(sorted(tag_index.items()))),
    lambda: visualize_syntax_clusters(syntax_pool, tag_index, use_streamlit=True),
)
...
if st.button("初回進化・発話"):
    ...
    if emitted_in_action:
        st.success("初回進化・発話が完了しました。発話構文を生成しました。")
    else:
        st.info("初回進化・発話は完了しましたが、発話構文はありませんでした。")
...
st.subheader("構文進化系譜")
_show_cached_figure(
    "sca_genealogy_figure",
    ("genealogy", _syntax_signature(syntax_pool + emitted)),
    lambda: draw_syntax_genealogy(syntax_pool + emitted, use_streamlit=True),
)
...
_show_cached_figure(
    "sca_cooccurrence_figure",
    ("cooccurrence", _syntax_signature(syntax_pool + emitted)),
    lambda: draw_tag_cooccurrence_network(syntax_pool + emitted, use_streamlit=True),
)
...
if st.button("スコア淘汰（<0.3）"):
    removed = mz.prune_by_score(min_score=0.3)
    if removed:
        st.success(f"スコアによる構文淘汰: {removed}件を削除しました。")
    else:
        st.info("スコアによる構文淘汰: 変化なし（対象なし）。")

if st.button(f"世代淘汰（{DEFAULT_MAX_AGE_GENERATIONS}世代超）"):
    removed = prune_memory_by_generation(
        mz, current_generation=st.session_state.sca_generation
    )
    if removed:
        st.success(f"世代差による構文淘汰: {removed}件を削除しました。")
    else:
        st.info("世代差による構文淘汰: 変化なし（対象なし）。")

if st.button("類似構文淘汰（閾値=0.9）"):
    removed = mz.prune_by_similarity(threshold=0.9)
    if removed:
        st.success(f"類似構文淘汰: {removed}件を削除しました。")
    else:
        st.info("類似構文淘汰: 変化なし（対象なし）。")